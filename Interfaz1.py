...
class Product:
    ...
    def __init__(self, window):
        ...
        cursor = cnx.cursor()
        #Creamos el diccionario que almacenará la información en formato JSON
        dict1 = {}

        #Clase de producto para el JSON
        class Producto:
            def __init__(self,cod,cant):
                self.cod = codigo
                self.cant = cantidad
        ...
        def exportarCantArt():
            ...
            for dato in table:
                print(dato['CODIGO'], dato['UNIACTUAL'])
                id_products = int(dato['CODIGO'])
                id_sucursal = 1
                cantidad = str(dato['UNIACTUAL'])
                
                add_art = ("INSERT INTO inventarios"
                            "(`id_products`, `id_sucursal`, `cantidad`) "
                            "VALUES (%s,%s,%s)")

                dato_art = (id_products,id_sucursal,cantidad)
        ...
        Button(frame, text = 'Seleccione el INSAxxxx.dbf', command = abrir_archivo).grid(row=2, column=1)
